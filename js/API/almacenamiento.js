...
function crearReservas(th,pr,ha,di){
	accesoBD().transaction(function(tx){
       ...
       navigator.notification.alert("Error processing SQL: "+err.code,null,"ok","Cancel");
...
function leerHistorial(){
	accesoBD().transaction(function(tx){
		tx.executeSql('SELECT * FROM historial',[],function(tx2,res){
			...
			//alert('No se leyó correctamente');
		navigator.notification.alert("No se leyó correctamente"+err.code,null,"ok","Cancel");

		});
	},function(err){
		//alert("Error processing SQL: "+err.code+"dato"+largo);
	navigator.notification.alert("Error processing SQL: "+err.code,null,"ok"+largo,"Cancel");
